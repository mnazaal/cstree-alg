...
from .utils.tools import undirected_both, coming_in, v_structure, data_to_contexts,  generate_vals, context_is_contained, parents
    ...
    for mc in min_contexts:
        ...
        for j in range(len(vars)+1):
            for k,l in combinations(vars, 2): #For each edge in the complete graph 
                ...
                """k_neighbours = list(set(list(dag.neighbors(k))).difference({k})) 
                #S_sizes = range(len(neighbours)+1)
                #for subset_size in S_sizes:
                subsets = [set(i) for i in combinations(k_neighbours, j)]
                zero_indexed_subsets = [set([i-1 for i in s]) for s in subsets]
                #print(k-1,l-1,zero_indexed_subsets)
                for subset in zero_indexed_subsets:
                    # For the CI testing, we need zero indexing
                    ...
                    if p_val > alpha:
                        ...
                        if ({l},{k}, subset) not in CI:
                            CI.append(({l},{k}, subset))
                l_neighbours = list(set(list(dag.neighbors(l))).difference({l})) 
                #S_sizes = range(len(neighbours)+1)
                #for subset_size in S_sizes:
                subsets = [set(i) for i in combinations(l_neighbours, j)]
                zero_indexed_subsets = [set([i-1 for i in s]) for s in subsets]
                #print(k-1,l-1,zero_indexed_subsets)
                for subset in zero_indexed_subsets:
                    # For the CI testing, we need zero indexing
                    ...
                    if p_val > alpha:
                        ...
                        if ({l},{k}, subset) not in CI:
                            CI.append(({l},{k}, subset))"""
        minimal_context_dags.append((mc, nx.DiGraph(dag), CI))
